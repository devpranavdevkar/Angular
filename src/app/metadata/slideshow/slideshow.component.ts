...
export class SlideshowComponent implements OnInit, AfterViewInit {
  ...
  showCategory() {
    ...
    this.commonService
      .apiCall('get', `/api/v1/admin/product/getBanner`)
      .subscribe(
        (data) => {
          if (data['success']) {
  ...
  ngAfterViewInit(): void {
    setTimeout(
      () => this.commonService.setComponentname('Slideshow Management'),
      0
    );
  }
  ...
  showData() {
    ...
    this.commonService
      .apiCall(
        'get',
        `/api/v1/admin/product/getBanner?limit=${this.tableSize}&pageNo=${this.currentPage}`
      )
      .subscribe(
        (data) => {
          if (data['success']) {
  ...
  onChangeIsActive(event, id) {
    ...
    console.log(event);
    this.commonService
      .apiCall('post', `/api/v1/admin/product/updateBanner/${id}`, data)
      .subscribe(
        (response) => {
  ...
  addslide() {
    if (this.slideForm.valid) {
      // this.credentials.is_active = this.credentials.is_active.value;
      ...
      this.commonService.loader(true);
      this.commonService
        .apiCall('post', '/api/v1/admin/product/createBanner', formData)
        .subscribe(
          (response) => {
            if (response['success']) {
              this.modalRef.hide();
  ...
  update(id) {
    ...
    this.commonService
      .apiCall('post', `/api/v1/admin/product/updateBanner/${id}`, formData)
      .subscribe(
        (response) => {
          this.showImageSelectionInput = false;
          this.showDescriptionImageSelectionInput = false;
  ...
  deleteslide(id: number) {
    this.commonService.loader(true);
    this.commonService
      .apiCall('delete', `/api/v1/admin/product/deleteBanner/${id}`)
      .subscribe(
        (response) => {
          if (response['success']) {
            this.showData();
